...
namespace FedeDiaz.ApplyPlayModeValues.Editor
{
    ...
    {
        private const string SessionKey = "ApplyPlayModeContextMenu.PendingItems";
        ...
        private static void ApplyPlayValuesMenu(MenuCommand command)
        {
            ...
            if (CaptureComponent(component))
                Debug.Log(
                    $"Captured '{component.name}'. Values will be applied when exiting Play Mode.",
                    component);
        }
        ...
        private static bool ApplyPlayValuesMenuValidate(MenuCommand command)
        {
            return Application.isPlaying
                && command.context is Component component
                && IsSceneSaved(component);
        }
        ...
        private static bool CaptureComponent(Component component)
        {
            if (component == null)
                return false;
            ...
            {
                Debug.LogWarning(
                    $"Cannot capture '{component.name}'. The scene must be saved first.",
                    component);
            ...
            {
                Debug.LogWarning(
                    $"Could not get a valid GlobalObjectId for '{component.name}'.",
                    component);
        ...
        private static void ApplyPendingItems()
        {
            ...
            if (applied > 0)
                Debug.Log($"Applied Play Mode values to {applied} component(s).");
        }
        ...
        private static bool ApplyPendingItem(PendingItem item)
        {
            ...
            {
                Debug.LogWarning($"Invalid GlobalObjectId: {item.globalObjectId}");
                return false;
            }
            ...
            {
                Debug.LogWarning($"Could not resolve object from GlobalObjectId: {item.globalObjectId}");
                return false;
            }

            if (targetObject is not Component targetComponent)
            {
                Debug.LogWarning($"Resolved object is not a Component: {targetObject.name}");
                return false;
            }
            ...
            {
                Debug.LogWarning($"Could not resolve component type: {item.typeAssemblyQualifiedName}");
                return false;
            }
            ...
            try
            {
                ...
                // Some built-in components (e.g. Transform) already exist on every
                // GameObject, so reuse the existing one instead of adding a duplicate.
                ...
                if (tempComponent == null)
                {
                    Debug.LogWarning($"Could not create a temporary '{componentType.Name}' to apply values.");
                    return false;
                }
